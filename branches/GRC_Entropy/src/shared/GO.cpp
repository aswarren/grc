...
//			getline(ss,CurrentFunc->name_space,'\n');
            ...
            int Instead;
            ss>>Instead;
            CurrentFunc->ConsiderInstead.push_back(Instead);
        }
        else if(tag=="consider"){//Currently, assumes all ID's are "GO:" prepended
            ss.ignore(80, ':');
            int Instead;
            ss>>Instead;
            CurrentFunc->ConsiderInstead.push_back(Instead);
        }
        else if(tag=="replaced_by"){//Currently, assumes all ID's are "GO:" prepended
            ss.ignore(80, ':');
            int Instead;
            ss>>Instead;
            ...
            if(rel=="is_a"){
                ...
                if(UnknownTag.find(rel)==UnknownTag.end()){
                    cerr<<"WARNING: Not Using Relationship \""<<rel<<"\" Example. Line:"<<linenum<<"\n";
                    cerr<<" >> "<<line<<"\n";
                    UnknownTag.insert(rel);
                }
            }
        ...
        //they are currently ignored but fair warning should be given to the user
            ...
            if(UnknownTag.find(tag)==UnknownTag.end()){
                cerr<<"WARNING: Unknown Tag \""<<tag<<"\" Found. Line:"<<linenum<<"\n";
                cerr<<" >> "<<line<<"\n";
                UnknownTag.insert(tag);
            }
        }
    }//close while
//	cerr<<"done parsing"<<"\n";
