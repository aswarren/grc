...
//The intent of the modifications is to prevent an error when an entire sequence is low complexity
//********************************MODIFIED*******************************************
#include "blast.h"
...
int4 main(int4 argc, char* argv[])
{
	...
	//MODIFIED BY ANDREW
	int Filtered=0;//Stores whether sequence has not been filtered to oblivion
        // Filter the query using DUST or SEG
        if (parameters_filterEnabled)
        {
            if (queryAlphabetType == encoding_protein)
                Filtered=seg_segSequence(query);//MODIFIED BY ANDREW
            else
...
//    	printf("Total malloced=%s\n", global_int4toString(global_totalMalloc));
	...
	hitMatrix_free();
	alignments_free();
	//PSSMatrix_free(PSSMatrix);//MODIFIED BY ANDREW
    //readdb_close();
