//********************************MODIFIED*******************************************
//This source file has been modified from its origninal version by Andrew Warren anwarren@vt.edu on 11/01/06
//Modified lines are those marked by "MODIFIED BY ANDREW"
//The intent of the modifications is to prevent an error when the entire sequence is low complexity
...
int seg_segSequence(char* sequence)
{
    int NumX=0;
    struct Sequence *seq;
    struct Segment *segs;
...
/*------------------------------------------------------------(singreport)---*/
//MODIFIED BY ANDREW
//Modified to return if there is only <6 letters that are not x's
int singreport(seq, segs)
	struct Sequence	*seq;
	struct Segment	*segs;
{
	char	*proseq;
...
/*	for (i=0, ctr=0; proseq < proseqmax; ++i, ++ctr, ++proseq) {
	...
	return total;//return total number of AA's converted to X's
