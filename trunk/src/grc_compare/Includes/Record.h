    ...
    int ParseHit(GO* GOAccess){//open definition
        ...
        while (Breakup>>Term){ //read in terms
            if(Term[0]=='(' && ConfEligible){
                ...
                FindIt->second.second=stod(Term);
            }
            else if(Term[Term.size()-1]==')' && ConfEligible){
                Term.erase(Term.size()-1,1);
                if(GO::IsECode(Term)){
            ...
            else if (GO::StringIsGO(Term)){//if the term is a go term
                ...
                ConfEligible=(FindIt!=GOTerms.end() && !GOTerms.empty());
            }//close go term
            else if (GO::IsECode(Term) && ConfEligible){//if its an evidence code
                FindIt->second.first.insert(Term);//insert evidence code
