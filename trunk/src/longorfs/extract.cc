...
//The intent of the modifications is simply to include the start/stop coordinates (in addition to the name) for each ORF
//in the header information for each sequence 
//********************************MODIFIED*******************************************
      ...
      if  ((Start < End && End - Start <= Len / 2) || Start - End > Len / 2)
          {
           ...
           if  (Gene_Len < Min_Gene_Len)
               continue;
           printf ("%c%-10s \t %u \t %u \n",'>', Name, Start, End);//MODIFIED BY ANDREW
           if  (Skip_Start_Codon)
               i = Start + 2;
             else
               i = Start - 1;
