...
//The intent of the modifications is simply to put the long-orf statistics about minimum overlap into a separate output file
//********************************MODIFIED*******************************************
...
int  main  (int argc, char * argv [])

  {
   ...
   //MODIFIED BY ANDREW
   stat = fopen ("GlimStats.out","w");
   fprintf (stat, "Minimum gene length = %ld\n", Min_Gene_Len);
   fprintf (stat, "Minimum overlap length = %d\n", Min_Olap);
   fprintf (stat, "Minimum overlap percent = %.1f%%\n\n", 100.0 * Min_Olap_Fraction);
   //putchar ('\n');
   fclose (stat);
   ...
   //modified by Andrew
   stat = fopen ("GlimStats.out","a");  
   fprintf (stat, "End = %ld\n", Data_Len);

   fprintf (stat, "\n\nPutative Genes are sent to std out:\n");
   fclose (stat);
